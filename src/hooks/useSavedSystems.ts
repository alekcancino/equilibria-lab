 ...
 * shareable URL — already kept in sync by useShareEffect/useShareableState
 * before the user can click anything, the same principle ShareButton relies
 * on — under a name; "loading" navigates to it, so restoration goes through
 * the exact same parsing/validation path every module already has for a
 * pasted share link (e.g. isValidAcidSystem), with no separate logic to keep
 * in sync.
 */
