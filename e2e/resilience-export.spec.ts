...
test('failed module chunk shows recoverable error and reloads after unblock', async ({ page }, testInfo) => {
...
test('theme toggle switches document data-theme', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'desktop', 'desktop-only smoke');
...
test('chart export menu downloads CSV data', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'desktop', 'desktop-only smoke');
